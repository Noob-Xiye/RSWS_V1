...
impl AuditLogService {
    ...
    pub async fn query(&self, query: AuditLogQuery) -> Result<AuditLogPage, RswsError> {
        ...
        if let Some(aid) = query.admin_id { count_query = count_query.bind(aid); }
        if let Some(a) = &query.action { count_query = count_query.bind(a); }
        if let Some(rt) = &query.resource_type { count_query = count_query.bind(rt); }
        if let Some(rid) = query.resource_id { count_query = count_query.bind(rid); }
        if let Some(rl) = &query.risk_level { count_query = count_query.bind(rl); }
        ...
        if let Some(aid) = query.admin_id { data_query = data_query.bind(aid); }
        if let Some(a) = &query.action { data_query = data_query.bind(a); }
        if let Some(rt) = &query.resource_type { data_query = data_query.bind(rt); }
        if let Some(rid) = query.resource_id { data_query = data_query.bind(rid); }
        if let Some(rl) = &query.risk_level { data_query = data_query.bind(rl); }
