...
-- RSWS 测试数据注入脚本
-- 用途：开发/测试环境初始化数据
-- 执行：psql -d rsws_db -f scripts/test_data.sql
-- ========================================
...
VALUES
  (1, 'user1@test.com',
   '\\=19\=19456,t=2,p=1\\',
   '测试用户1', true, true, NOW(), NOW()),
  (2, 'user2@test.com',
   '\\=19\=19456,t=2,p=1\\',
   '测试用户2', true, true, NOW(), NOW()),
  (3, 'seller@test.com',
   '\\=19\=19456,t=2,p=1\\',
   '测试卖家', true, true, NOW(), NOW())
ON CONFLICT (id) DO NOTHING;
...
-- ========== 用户收款配置 ==========
INSERT INTO user_payment_configs (id, user_id, config_name, payment_method,
  paypal_email, usdt_address, usdt_network, is_active, is_default, created_at, updated_at)
VALUES
  (1, 3, '我的 PayPal', 'paypal', 'seller@test.com', NULL,       NULL,     true, true,  NOW(), NOW()),
  (2, 3, 'USDT 收款',  'usdt',   NULL,            'TQn8KjVLwJGJXHxX5JHxE59ZmFqM9Rggg', 'tron', true, false, NOW(), NOW())
ON CONFLICT (id) DO NOTHING;

SELECT setval('user_payment_configs_id_seq', COALESCE((SELECT MAX(id) FROM user_payment_configs), 0) + 1, false);

-- ========== 资源收款配置 ==========
INSERT INTO resource_payment_configs (resource_id, payment_config_id, is_active, created_at)
VALUES
  (5, 1, true, NOW()),
  (5, 2, true, NOW()),
  (6, 1, true, NOW()),
  (7, 1, true, NOW())
ON CONFLICT (resource_id, payment_config_id) DO NOTHING;
...
VALUES
  ...
  (11, 7,    '平台订单',   'Order',       '/admin-orders',     'AdminOrder',      'order:read',    34, true, NOW(), NOW()),
...
-- ========== 示例订单 ==========
INSERT INTO orders (id, user_id, resource_id, amount, status, payment_method, created_at, updated_at)
VALUES
  (1, 1, 1,  0.00, 'completed', 'free',          NOW() - INTERVAL '7 days', NOW() - INTERVAL '7 days'),
  (2, 1, 3, 29.99, 'completed', 'paypal',        NOW() - INTERVAL '3 days', NOW() - INTERVAL '3 days'),
  (3, 2, 4, 49.99, 'paid',      'usdt_trc20',    NOW() - INTERVAL '1 day',  NOW() - INTERVAL '1 day'),
  (4, 1, 6, 19.99, 'pending',   'paypal',        NOW(),                NOW())
ON CONFLICT (id) DO NOTHING;

SELECT setval('orders_id_seq', COALESCE((SELECT MAX(id) FROM orders), 0) + 1, false);

-- ========== 佣金规则 & 记录 ==========
INSERT INTO commission_rules (id, name, rule_type, rate, is_active, created_at, updated_at)
VALUES
  (1, '默认佣金', 'default', 0.0500, true, NOW(), NOW())
ON CONFLICT (id) DO NOTHING;

SELECT setval('commission_rules_id_seq', COALESCE((SELECT MAX(id) FROM commission_rules), 0) + 1, false);

INSERT INTO commission_records (order_id, user_id, referrer_id, rule_id,
  order_amount, commission_amount, commission_rate, status, created_at)
VALUES
  (2, 1, 3, 1, 29.99, 1.50, 0.0500, 'pending', NOW() - INTERVAL '3 days')
ON CONFLICT DO NOTHING;

-- ========== 完成提示 ==========
DO \$\$ BEGIN
   RAISE NOTICE '✅ RSWS 测试数据注入完成！';
   RAISE NOTICE '管理员: admin@rsws.com / Admin123';
   RAISE NOTICE '测试用户: user1@test.com / user2@test.com / seller@test.com (密码均为 Admin123)';
END \$\$;
