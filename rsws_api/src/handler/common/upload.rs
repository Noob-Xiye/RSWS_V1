    ...
    fn from(err: multer::Error) -> Self {
        match err {
            ...
            multer::Error::NoBoundary => {
                UploadError::BoundaryParseFailed("未找到 boundary".to_string())
            }
            ...
            multer::Error::FieldSizeExceeded { limit, field_name } => {
                ...
                UploadError::FileSizeExceeded(format!(
                    "字段 {} 超过大小限制: {} bytes",
                    name, limit
                ))
            }
            ...
            multer::Error::IncompleteStream => {
                UploadError::MultipartParseError("不完整的 multipart 流".to_string())
            }
            multer::Error::IncompleteHeaders => {
                UploadError::MultipartParseError("不完整的字段头".to_string())
            }
            ...
            multer::Error::ReadHeaderFailed(e) => {
                UploadError::MultipartParseError(format!("读取头失败: {:?}", e))
            }
            multer::Error::DecodeHeaderName { name, .. } => {
                UploadError::MultipartParseError(format!("解码头名失败: {:?}", name))
            }
            multer::Error::DecodeHeaderValue { .. } => {
                UploadError::MultipartParseError("解码头值失败".to_string())
            }
            multer::Error::DecodeContentType(_) => UploadError::InvalidContentType,
            multer::Error::LockFailure => {
                UploadError::MultipartParseError("锁定 multipart 状态失败".to_string())
            }
...
pub async fn init_upload(
    ...
    info!(
        "Upload initialized: upload_id={}, file_key={}, chunks={}",
...
pub async fn upload_chunk(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    ...
    let session_key = format!("upload_session:{}", upload_id);
    let mut session: UploadSession = match state
        .config_service
        .redis_client()
    ...
    if let Err(e) = storage_service
        .upload(&chunk_key, chunk_data, session.content_type.as_deref())
        .await
    {
    ...
    info!(
        "Chunk uploaded: upload_id={}, chunk={}/{}",
        upload_id,
        chunk_index + 1,
    ...
    res.success_msg(
        (),
        format!("分块 {}/{} 上传成功", chunk_index + 1, session.total_chunks),
    );
}
...
pub async fn complete_upload(
    ...
    let session: UploadSession = match state
        .config_service
        .redis_client()
        .get_json(&session_key)
        .await
    ...
    let file_url = if session.total_chunks == 1 {
        ...
        let result = match storage_service
            .upload(&session.file_key, data, session.content_type.as_deref())
            .await
        {
            Ok(r) => r,
        ...
        for i in 0..session.total_chunks {
            ...
            let chunk_data = match storage_service.download(&chunk_key).await {
                Ok(d) => d,
                Err(e) => {
                    return res.error(RswsError::internal(format!("下载分块 {} 失败: {}", i, e)))
                }
            };
    ...
    info!(
        "Upload completed: upload_id={}, file_url={}",
        req.upload_id, file_url
    );
...
/// 单文件上传
#[endpoint]
pub async fn upload_single(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    let state = get_state(depot);
    ...
    let result = match storage_service
        .upload(&file_key, file_data.clone(), content_type.as_deref())
        .await
    {
        Ok(r) => r,
    ...
    info!(
        "Single file uploaded: file_key={}, url={}",
        file_key, result.url
    );
...
async fn read_multipart_file(
    req: &mut Request,
) -> Result<(Bytes, String, Option<String>), UploadError> {
    // 获取 content-type 头
