...
use rsws_model::payment::{CreatePayPalConfigRequest, UpdatePayPalConfigRequest};
use salvo::prelude::*;
...
/// 创建 PayPal 配置
#[endpoint(
    request_body = CreatePayPalConfigRequest,
    responses(
...
pub async fn create_paypal_config(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    let body: rsws_model::payment::CreatePayPalConfigRequest = match req.parse_json().await {
        Ok(b) => b,
        Err(e) => {
            res.error(RswsError::bad_request(format!("Invalid request: {}", e)));
            return;
        }
    };
